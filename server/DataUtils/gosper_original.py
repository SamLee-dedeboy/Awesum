"""
The Gosper curve, also known as Peano-Gosper Curve, named after Bill
Gosper, also known as the flowsnake (a spoonerism of snowflake), is
a space-filling curve.

It is a fractal object similar in its construction to the dragon curve
...
def plot_level(max_level = 6, **kwargs):
    ...
    if showAllLevel:
        ...
        for i in range(max_level, -1, -1):
            ...
            ax.plot(fAdd( (+7.0**0.5 + x_offset) * i, xr), yr,
                    linewidth=0.5, color='C' + str(i))
        ax.set_aspect(1)
        ...
        if tile:
            scale = +7.0**0.5
            for i in range(-2, 3):
                ax.plot(fAdd(i * scale, xr), yr,
                        linewidth=0.5, color='C' + str(i + 2))
            for i in range(-2, 2):
                ax.plot(fAdd(+scale*(i+0.5), xr),
                        fAdd(+scale*3**0.5*0.5,yr),
                        linewidth=0.5, color='C' + str(i + 5))
                ax.plot(fAdd(+scale*(i+0.5), xr),
                        fAdd(-scale*3**0.5*0.5,yr),
                        linewidth=0.5, color='C' + str(i + 5))
...
def create_animated_gif(maxRecursionLevel=6, filename='gosper_curve.gif', **kwargs):
    ...
    for level in generateLevel(maxRecursionLevel + 1):
        ...
        cmd += cfilename + ' '
        plot_level(max_level=level, showAllLevel=False,
                   filename=cfilename, tile=tile, grid=grid)
    cmd += filename
...
def main():
    ...
    parser = ArgumentParser(description='Generate a gosper fractal curve')
    pa = parser.add_argument
    pa('level', type=int,
        help='number of recursion level. Reasonnable value is 6')
    pa('-t','--tile', action='store_true',
        help='boolean used to create a tiling of the generated curve')
    pa('-a', '--all', action='store_true',
        help='boolean used to display all levels (disable when tiling)')
    pa('-o', '--output', default=None,
        help='name of the generated file. If not provided, '
             + 'result will display on screen')
    pa('-g', '--grid', action='store_true',
        help='boolean used to display grid')
    args = parser.parse_args()
    if args.output and args.output.lower().endswith('gif'):
        create_animated_gif(maxRecursionLevel=args.level,
                            filename=args.output,
                            grid=args.grid,
                            tile=args.tile)
    else:
        plot_level(args.level,
                   showAllLevel=args.all,
                   filename=args.output,
                   grid=args.grid,
                   tile=args.tile)
