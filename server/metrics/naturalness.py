...
def get_meta_data(df): #change to LLM for LLM meta data
    mean_sd = [(np.std(df['LLM Average Dependency tree heights']),np.mean(df['LLM Average Dependency tree heights'])),
        (np.std(df['LLM_average_sentence_lengths']),np.mean(df['LLM_average_sentence_lengths'])),
        (np.std(df['LLM_avg_left_subtree_height']),np.mean(df['LLM_avg_left_subtree_height'])),
        (np.std(df['LLM_avg_right_subtree_height']),np.mean(df['LLM_avg_right_subtree_height']))]
    if 'naturalness_score_llm' in df.columns:
        Q1 = np.percentile(df['naturalness_score_llm'], 25)
        Q2 = np.percentile(df['naturalness_score_llm'], 50)
        Q3 = np.percentile(df['naturalness_score_llm'], 75)
        range1 = (df['naturalness_score_llm'].min(), Q1)
        range2 = (Q1,Q2)
        range3 = (Q2,Q3)
        range4 = (Q3, df['naturalness_score_llm'].max())
